...
router
    ...
    .post('/search/location', async function (req, res, next) {
        ...
        fetch(`${getLatLangAPI}?key=${apiKey}&q=${req.body.location}`, requestOptions)
            .then(response => response.json())
            .then(result => {
                fetch(`${weatherAPI}?lat=${result.results[0].geometry.lat}&lon=${result.results[0].geometry.lng}`, {
                    method: 'GET',
                    headers: {
                    ...
                    .then(response => {
                        resp = response;
                        const time = moment(response.now_dt).format('MMMM Do YYYY, h:mm:ss a');
    ...
    .post('/search/location/save', async (req, res) => {
        ...
        if (forecastDB.length !== 0) {
            console.log('You already saved this city!');
        } else {
            const forecast = new Forecast({name, date_time, temp, icon, condition, feels_like});
            forecast.save();
        }
    })
